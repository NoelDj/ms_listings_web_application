...
def user_detail(request, username):
    ...
    if request.method == 'PUT':
        serializer = UserSerializer(instance=queryset, data=request.data, partial=True, context={'request': request})
        if serializer.is_valid():
            serializer.save()
            return Response({'message': f'User: {username} has been updated', 'listing': serializer.data}, status=status.HTTP_200_OK)
        return Response({'message': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
...
def listing_detail(request, id):
    ...
    if request.method == 'DELETE':
        ...
        return Response({'message' : f'Listing: {id} has been deleted'}, status=status.HTTP_200_OK)
    ...
    if request.method == 'PUT':
        ...
        serializer = ListingSerializer(queryset, data=request.data, context={'request': request})
        if serializer.is_valid():
            serializer.save(images=images,files=files,remove_images=remove_images,remove_files=remove_files)
            return Response({'message': f'Listing {id} has been updated', 'listing': serializer.data}, status=status.HTTP_200_OK)
        else:
...
def listings_collection(request):
    if request.method == 'GET':
        ...
        queryset = Listing.filter_listings(search_param=search_param, owner_param=owner_param, category_param=category_param).order_by('-created_at')
        paginated_queryset = paginator.paginate_queryset(queryset, request)
...
def comment_detail(request, id):
    ...
    if request.method == 'GET':
        ...
        serializer = CommentSerializer(queryset, data=request.data, context={'request': request})

        if serializer.is_valid():
            serializer.save()
            return Response({'message' : f'Comment: {id} has been updated', 'listing': serializer.data}, status=status.HTTP_200_OK)
        return Response({'message': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)


    elif request.method == 'DELETE':
        queryset.delete()
        return Response({'message' : f'Comment: {id} has been deleted'}, status=status.HTTP_200_OK)
...
def likes_collection(request):
    ...
    if request.method == 'GET':
        ...
        return Response({'like': serializer.data, 'user_likes_listing': user_likes_listing}, status=status.HTTP_200_OK)
    if request.method == 'POST':
        listing_id = request.POST.get('listing_id')
        user = request.user
        existing_like = Like.objects.filter(user=user, listing=listing).exists()
        if existing_like:
            return Response({'message': 'User already likes listing.'}, status=status.HTTP_400_BAD_REQUEST)
...
def like_detail(request, id):
    ...
    if request.method == 'DELETE':
        like.delete_listing()
        return Response({'message' : f'Like: {id} has been deleted'}, status=status.HTTP_200_OK)